...
const Breadcrumbs = ({ categories, product }) => {
    ...
    return <ul className="mb-8 text-sm text-gray-500 list-none inline-flex items-center space-x-1">
        {category.breadcrumbs.map(({ category_name, category_url_path }, index ) => <li key={`breadcrumb-${index}`} className="inline-flex items-center">
            <Link href={`/catalog/category/${category_url_path}`}>
                <a className="pr-2">{category_name}</a>
            </Link>
            {arrow}
        </li>)}
        <li className="inline-flex items-center">
            <Link href={`/catalog/category/${category.url_path}`}>
